"""Shared visual style for generated figures.

Defines typography, color, grid, legend, and export conventions for all
matplotlib figures generated by the package.
...
# Model colors/labels and product color come from config.
...
# Usable text width for A4 manuscript figures.
...
# Realized series and reference lines use neutral colors, not model colors.
...
# Sans-serif fallback stack across operating systems.
...
def setup_matplotlib_thesis() -> None:
    ...
    Sets a non-interactive backend, neutral grid, sans-serif typography, and a
    transparent background for PDF export.
    """
...
def model_boxplot(ax: Any, data: list, models: list[str], *, ref: float = 1.0):
    """Styled boxplot with canonical model colors and optional reference line."""
    if ref is not None:
...
def save_figure(fig: Any, name: str, output_dir: Path | str) -> Path:
    """Save a figure as vector PDF with a transparent background.
...
def panel_3x2(figsize_scale: float = 1.0, *, ratio: float = 0.92) -> Any:
    """Create a standardized 3x2 panel (municipality x tax).
